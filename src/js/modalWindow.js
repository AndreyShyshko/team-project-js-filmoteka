...
function addClassOnBtn(id) {                                                        // проверяет наличие ID фильма в локал сторедж и добавляет клас активной кнопки
...
function fetchMoviesForIdByModal(movieId) {                                        // ищет фильмы по ID и добавляет розметку в gallery
  const url =
    `https://api.themoviedb.org/3/movie/${movieId}?api_key=219747bddc830c6768a55001e81d80ed`;
...
function saveFilmToWatched(id) {                           // записывает id фильма в локал сторедж "Просмотренные"
...
function saveFilmToQueue(id) {                              // записывает id фильма в локал сторедж "Добавленных в очеридь"
